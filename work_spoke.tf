locals {
  ...
  work_subnets = {
    WorkSubnet    = cidrsubnet(local.work_vnet_address_prefix, 8, 0) #10.1.0.0/24
    StorageSubnet = cidrsubnet(local.work_vnet_address_prefix, 8, 1) #10.1.1.0/24
    AKSSubnet     = cidrsubnet(local.work_vnet_address_prefix, 8, 2) #10.1.2.0/24
  }
}
